...
export async function resolveImplicitProviders(params: {
  ...
  // ShengSuanYun provider - always available to show model options during onboarding.
  // Model list includes defaults that work without API discovery.
  const shengsuanyunKey =
    resolveEnvApiKeyVarName("shengsuanyun") ??
    resolveApiKeyFromProfiles({ provider: "shengsuanyun", store: authStore });
  // Always build the provider (even without API key) so models show during onboarding
  providers.shengsuanyun = {
    ...(await buildShengSuanYunProvider()),
    ...(shengsuanyunKey ? { apiKey: shengsuanyunKey } : {}),
  };
  const togetherKey =
